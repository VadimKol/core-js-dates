...
function isDateInPeriod(date, period) {
  return (
    new Date(period.end) - new Date(date) >= 0 &&
    new Date(period.start) - new Date(date) <= 0
  );
}
...
function formatDate(date) {
  ...
  let midnight = '';
  let hours = formatedDate.getUTCHours();
  let minutes = formatedDate.getUTCMinutes();
  let seconds = formatedDate.getUTCSeconds();
  if (minutes < 10) minutes = `0${minutes}`;
  if (seconds < 10) seconds = `0${seconds}`;
  if (hours >= 12) {
  ...
  return `${formatedDate.getUTCMonth() + 1}/${formatedDate.getUTCDate()}/${formatedDate.getUTCFullYear()}, ${time} ${midnight}`;
}
...
function getCountWeekendsInMonth(month, year) {
  ...
  if (daysInMonth === 31) {
    if (dayOfTheWeek === 5 || dayOfTheWeek === 6) return 10;
    if (dayOfTheWeek === 0 || dayOfTheWeek === 4) return 9;
  }
  if (daysInMonth === 30) {
    if (dayOfTheWeek === 6) return 10;
    if (dayOfTheWeek === 0 || dayOfTheWeek === 5) return 9;
  }
  if (daysInMonth === 29 && (dayOfTheWeek === 0 || dayOfTheWeek === 6))
    return 9;
  return 8;
}
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  ...
  while (endDate - startDate >= 0) {
    workingDays.push(getNormalDate(startDate));
    workDays -= 1;
    if (workDays === 0) {
